...
\author{\textbf{队伍名：Attention请注意我} \\ 郑亦航 PB23111 \\ 李梓煊 PB23111709}
\date{\today}
...
\begin{abstract}
本队在 NLP 新闻分类学习赛中以准确率 0.962 获得第 3 名，成绩排名截图已置于文末附录以供查阅。
\end{abstract}
...
\begin{itemize}
    ...
    \begin{enumerate}
        ...
        \item \textbf{类别极度不平衡}：训练集中类别数量分布长尾明显，最大与最小类别比例高达 43:1，模型在标准训练中容易偏向头部类别。
...
\begin{itemize}
    ...
    \item \textbf{策略启示}：如此悬殊的比例要求我们在 Loss 设计上引入类别平衡手段（如 Class Weighting 或 Focal Loss），否则模型将过度偏向样本量多的头部类别。
\end{itemize}
...
\begin{itemize}
    \item 0: \texttt{[PAD]}
    \item 1: \texttt{[UNK]}
    \item 2: \texttt{[CLS\_SEG]}（用于段级聚合）
    \item 3: \texttt{[SEP]}
    \item 4: \texttt{[MASK]}（用于预训练）
\end{itemize}
...
本赛题的核心是在有限的算力和时间内处理大规模长文本分类问题。我们的方案决策基于以下关键约束：
\begin{itemize}
    \item \textbf{数据约束}：共有 20 万样本，约 2 亿 token。由于数据为匿名 ID，无法利用现有预训练模型（如 BERT/LLaMA）的语义知识，必须\textbf{从零开始预训练}模型。
    \item \textbf{算力约束}：可用硬件为 4 卡 H800 GPU，预训练时间窗口约为 24 小时。
...
\begin{table}[H]
    ...
    \begin{tabular}{p{4cm} p{4cm} p{4cm}}
        ...
        \midrule
        \textbf{大语言模型} \newline (LLaMA/Qwen 等) & 强大的生成能力与 few-shot 学习能力。 & 参数量庞大（7B+），需海量数据从零训练，区区 2 亿 token 容易导致过拟合；训练代价高昂。 \\
        \midrule
        \textbf{截断版 BERT} \newline (Max Len 512) & 训练速度快，基础完善。 & 丢失 50\% 以上的信息（尤其是新闻/法律文本的尾部结论），性能上限受限。 \\
        \midrule
        \textbf{稀疏 Transformer} \newline (Longformer/BigBird) & 原生支持长上下文，复杂度 $O(N)$。 & 需自定义 CUDA Kernel 或复杂实现；在 4k 长度下显存优势不明显。 \\
        \midrule
        \textbf{层次化 Transformer} \newline (HAT/HDT) & \textbf{性价比最高}：段内标准 Attention 提取特征，段间聚合提供全局视野；显存占用低。 & 需要定制分层结构（段内注意力 + 跨段注意力）。 \\
        \bottomrule
    \end{tabular}
...
\begin{itemize}
    ...
    \item 将每篇长文档切分为最多 $N$ 个片段，每段长度固定为 $K=512$，以尽可能覆盖全文。
    \item 我们以 15\% 的概率将部分 token 进行 Mask，并对输入和标签进行 padding，以对齐批处理的形状。
\end{itemize}

\paragraph{技术手段}

预训练阶段，我们基于 HAT-MLM 架构，结合 AdamW 优化器、线性 warmup + 学习率衰减、EMA 等技术，构建了一条稳定可复现的预训练流程，主要配置如下：
\begin{itemize}
    \item 模型采用 HAT-MLM 架构，默认 batch size 为 4，总训练步数约 10000 步。数据读入并打乱后按 9:1 划分训练集和验证集。
    \item 优化器为 AdamW，初始学习率 5e-5，权重衰减 0.01，$\texttt{betas}=(0.9, 0.999)$，$\texttt{eps}=1\text{e-}8$，反向传播中使用梯度裁剪（阈值 1.0）。
...
使用预处理后的新闻文本及其标签作为训练语料。对于每篇长文档，按固定长度将其切分成若干片段，并在批处理阶段打包成形状为 \([B,N,K]\) 的张量输入模型。分类模型基于 HAT 架构实现，内部会自动添加 \texttt{[CLS]} 标记并输出文档级 logits；模型初始化优先加载对应的预训练参数。

\paragraph{损失函数与优化策略}

训练过程中综合使用了多种分类损失，包括交叉熵、标签平滑以及 Focal Loss 及其组合。默认标签平滑系数为 0.05，Focal Loss 的 $\gamma=2.0$，并结合预先计算的类别权重以及可选的样本加权采样，以缓解类别不平衡。batch size 默认为 64（训练）和 128（验证），反向传播阶段采用梯度裁剪（阈值 1.0）。
...
\begin{table}[h]
    ...
    \begin{tabular}{ccc}
        ...
        \hline
        \textbf{平均} & \textbf{0.9602 ± 0.0019} & （存储于 \texttt{checkpoints/cls\_hat512\_kfold}） \\
        \hline
    \end{tabular}
\end{table}
...
在实际实验中，Stage2 的 Macro-F1 相较 Stage1 整体提升不明显，部分折甚至略有下降。最终提交结果中我们仍主要采用 Stage1 权重。初步分析原因包括：
...
从表~\ref{tab:stage2_metrics} 可以看出，整体上 Stage2 未能超越 Stage1，且在 Fold 3 进行到第 2 个 epoch 中途作业中止，后续折未继续尝试；因此最终推理仍沿用 Stage1 的 5 个折模型。分析其原因：
\begin{itemize}
...
\begin{itemize}
    \item 多卡启动：当检测到需要使用多个 GPU 时，推理脚本将自动使用 \texttt{torchrun} 启动多进程，并通过 PyTorch 分布式接口（如 \texttt{init\_process\_group}）完成通信初始化，每个进程绑定到对应的 GPU。
    \item 数据划分：推理阶段采用分布式采样（DistributedSampler），将滑窗后的数据按进程划分，保证各进程处理不同的样本且不改变整体顺序，避免重复计算。验证集的校验也使用同样的分布式数据管道。
    \item 结果聚合：每个进程独立完成其负责样本的窗口聚合和模型集成，得到局部的预测结果（\texttt{doc\_id → logits/label}）。随后通过 \texttt{all\_gather\_object} 收集所有进程的结果到主进程（rank0），再统一生成提交文件（可选同时保存 logits）。这种设计避免了跨进程的频繁通信，仅在最后一步汇总结果。
    \item 容错与配置：当未指定多进程参数时，推理脚本会自动以单进程方式运行以避免误用。在集群环境下，我们根据节点的 GPU 数自动设置进程数，并显式配置通信地址、库路径和显存分配策略，以确保多 GPU 推理的稳定性。
\end{itemize}
